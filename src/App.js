...
export const StyledButton = styled.button`
  ...
  -webkit-box-shadow: 4px 4px 8px -2px rgba(0, 0, 0, 1);
  -moz-box-shadow: 4px 4px 8px -2px rgba(0, 0, 0, 1);
  :active {
    box-shadow: none;
...
function App() {
  ...
  const claimNFTs = () => {
    ...
    if (data.totalSupply < 1000) {
      var cost = data.cost / 10;
    }
    else if (data.totalSupply < 2000) {
      var cost = data.cost;
    }
    else if (data.totalSupply < 3000) {
      var cost = data.cost * 10;
    }
    else if (data.totalSupply < 5000) {
      var cost = data.cost * 50;
    }
    else if (data.totalSupply < 7000) {
      var cost = data.cost * 100;
    }
    else if (data.totalSupply < 8000) {
      var cost = data.cost * 500;
    }
    else if (data.totalSupply < 10000) {
      var cost = data.cost * 1000;
    }
    else {
      var cost = data.cost;
    }
      ...
      >
        ...
        <ResponsiveWrapper flex={1} style={{ padding: 24 }} test>
          <s.Container flex={1} jc={"center"} ai={"center"}>
            <StyledImg alt={"example"} src={"/config/images/logo2.png"} />
          </s.Container>
          <s.SpacerLarge />
          <s.Container
            flex={2}
